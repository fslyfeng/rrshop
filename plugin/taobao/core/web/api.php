...
class Api_EweiShopV2Page extends PluginWebPage
{
	...
	private function api_taobao($cath_id = 0, $catch_url = '', $pageSource = '')
	{
		...
		if (!empty($pageSource)) {
			...
			$value = array();
			$params = pq('.attributes-list li');

			if (!empty($params)) {
				foreach ($params as $li) {
					if (pq($li)->text() != '') {
	...
	private function api_alibaba($cath_id = 0, $catch_url = '', $pageSource = '')
	{
		...
		if (!empty($pageSource)) {
			$pageSource = html_entity_decode($pageSource);
			preg_match('/widget-wap-detail-common-footer[\\s\\S]*?data-module-hidden-data-area="Y">([\\s\\S]*?)</', $pageSource, $res);
			...
			foreach (pq('img') as $img) {
				...
				if ($img->getAttribute('swipe-lazy-src') != '') {
					pq($img)->attr('src', $img->getAttribute('swipe-lazy-src'));
					pq($img)->removeAttr('swipe-lazy-src');
				}
			...
			preg_match('/detailUrl\\":\\s\\"(.*?)"/', $pageSource, $urlArr);
			$detailUrl = $urlArr[1];
			$detailRequest = ihttp_request($detailUrl);
			preg_match('/offer_details={\\"content\\":\\"([\\s\\S]*?)\\"};/', $detailRequest['content'], $detailMatch);
			$detail['content'] = str_replace('\\', '', iconv('GBK', 'UTF-8', $detailMatch[1]));
			$thumb_url = array();

			foreach (pq('#J_Detail_ImageSlides img') as $img) {
				$url = pq($img)->attr('src');
				$thumb_url[] = $this->save_image($url, 1);
			}

			$thumb = $thumb_url[0];
			unset($thumb_url[0]);
			$priceRange = explode('-', $json1['displayPrice']);
			$minprice = floatval($priceRange[0]);
			$maxprice = empty($priceRange[1]) ? floatval($priceRange[0]) : floatval($priceRange[1]);
			...
			$detail['content'] = m('common')->html_to_images($detail['content']);
			$title = pq('title')->html();
			$data = array('uniacid' => $_W['uniacid'], 'thumb' => $thumb, 'thumb_url' => serialize($thumb_url), 'title' => $title, 'status' => 0, 'marketprice' => $maxprice, 'originalprice' => $minprice, 'minprice' => $minprice, 'maxprice' => $maxprice, 'hasoption' => $hasoption, 'createtime' => time(), 'total' => $json1['totalCanBookCount'], 'content' => $detail['content'], 'merchid' => $merchid, 'cates' => '', 'checked' => empty($merchid) ? 0 : 1, 'newgoods' => 1);
			...
			$value = array();
			$param_attribute = pq('.J_AttributeBtn .takla-item-text span');

			if (!empty($param_attribute)) {
				foreach ($param_attribute as $item) {
					$name = pq($item)->attr('data-offer-attribute-name');
					$attribute = pq($item)->attr('data-offer-attribute-value');
