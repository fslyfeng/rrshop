...
class Group_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		$psize = 10;
		$pstart = $psize * ($page - 1);
		$list = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . (' AND mode = 6 AND uniacid=:uniacid ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize), array(':uniacid' => $_W['uniacid']));
	...
	public function nostart()
	{
		...
		$psize = 10;
		$pstart = $psize * ($page - 1);
		$list = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_exchange_group') . ' WHERE unix_timestamp(starttime) >' . time() . (' AND mode = 6 AND uniacid=:uniacid ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize), array(':uniacid' => $_W['uniacid']));
		$allEnd = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_exchange_group') . ' WHERE unix_timestamp(endtime) <' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 6 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allStart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 6 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allNostart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_exchange_group') . ' WHERE  unix_timestamp(starttime) >' . time() . ' AND mode = 6 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	...
	public function post()
	{
		...
		global $_GPC;
		$id = $_GPC['id'];
		$t1 = tablename('ewei_shop_coupon');
		$t2 = tablename('ewei_shop_coupon_category');
	...
	public function add()
	{
		...
		if ($_W['ispost']) {
			...
			if (in_array(3, $select)) {
				...
				if ($red_type == 1) {
					if (intval($red) < 0.29999999999999999) {
						show_json(0, '红包面值不能小于0.3元');
					}
				}
				else {
					if ($red_type == 2) {
						if (intval($red_left) < 0.29999999999999999 || intval($red_right) < 0.29999999999999999) {
							show_json(0, '红包面值不能小于0.3元');
						}
					}
				}
	...
	public function edit()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($title) && !empty($endtime) && !empty($starttime) && $select != false) {
				$data = array('title' => $title, 'endtime' => $endtime, 'starttime' => $starttime, 'type' => $type, 'value' => $value, 'mode' => 6, 'status' => 1, 'uniacid' => $_W['uniacid'], 'score' => $score, 'max' => $max, 'coupon' => $coupon, 'balance' => $balance, 'red' => $red, 'red_left' => $red_left, 'red_right' => $red_right, 'balance_left' => $balance_left, 'balance_right' => $balance_right, 'score_left' => $score_left, 'score_right' => $score_right, 'score_type' => $score_type, 'balance_type' => $balance_type, 'red_type' => $red_type, 'coupon_type' => $coupon_type, 'goods' => $json_goods, 'binding' => $binding, 'postage' => $postage, 'postage_type' => $postagetype, 'sendname' => $sendname, 'wishing' => $wishing, 'actname' => $actname, 'remark' => $remark, 'goods_native' => in_array(1, $select) ? $goods_native : '');
	...
	public function creat()
	{
		...
		if (0 < intval($_GPC['ajax']) && 0 < intval($_GPC['loop'])) {
			...
			while (0 < $num) {
				...
				$insert = array('uniacid' => $_W['uniacid'], 'name' => 'ewei_shopv2:exchange:group', 'module' => 'reply', 'displayorder' => $rand_id, 'status' => 1);
...
';
				...
				$qrcode = array('uniacid' => $_W['uniacid'], 'acid' => $_W['uniacid'], 'type' => 'scene', 'extra' => 0, 'qrcid' => $scene, 'name' => '组合兑换', 'keyword' => md5($key), 'model' => 6, 'ticket' => $ticket['ticket'], 'url' => $ticket['url'], 'expire' => $ticket['expire_seconds'], 'subnum' => 0, 'createtime' => time(), 'status' => 1, 'scene_str' => '');
			...
			if (!empty($id)) {
				...
				if ($goods['goods'] != false) {
					foreach ($goods['goods'] as $k => $v) {
						$all[$k] = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_goods') . ' WHERE uniacid = :uniacid AND deleted = 0         AND hasoption = 0 AND id = :id', array(':id' => $v, ':uniacid' => $_W['uniacid']));
					}
				}
	...
	public function search()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_exchange_group') . ' WHERE mode = 6 AND uniacid=:uniacid  ' . $like . (' ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize), array(':uniacid' => $_W['uniacid']));
		$count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_exchange_group') . ' WHERE mode = 6 AND uniacid=:uniacid  ' . $like, array(':uniacid' => $_W['uniacid']));
	...
	public function banner()
	{
		...
		pdo_update('ewei_shop_exchange_group', array('banner' => $jsonBanner), array('uniacid' => $_W['uniacid'], 'id' => $id));
		show_json(1, '保存成功');
	}
