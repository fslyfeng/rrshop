...
class Record_EweiShopV2Page extends PluginWebPage
{
	...
	public function daifahuo()
	{
		...
		global $_GPC;
		$status = 1;
		$orderData = $this->orderData(1, 'daifahuo');
	}
	...
	public function daishouhuo()
	{
		...
		global $_GPC;
		$status = 2;
		$orderData = $this->orderData(2, 'daishouhuo');
	}
	...
	public function daifukuan()
	{
		...
		global $_GPC;
		$status = 0;
		$orderData = $this->orderData(0, 'daifukuan');
	}
	...
	public function yiwancheng()
	{
		...
		global $_GPC;
		$status = 3;
		$orderData = $this->orderData(3, 'yiwancheng');
	}

	public function yiguanbi()
	{
		global $_W;
		global $_GPC;
		$status = -1;
		$orderData = $this->orderData(-1, 'yiguanbi');
	}
	...
	public function ajaxorder()
	{
		global $_W;
		global $_GPC;
		$daifahuo = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' WHERE ordersn LIKE \'DH%\' AND uniacid = :uniacid AND status = 1', array(':uniacid' => $_W['uniacid']));
		$weifukuan = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' WHERE ordersn LIKE \'DH%\' AND uniacid = :uniacid AND status = 0', array(':uniacid' => $_W['uniacid']));
		$daishouhuo = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' WHERE ordersn LIKE \'DH%\' AND uniacid = :uniacid AND status = 2', array(':uniacid' => $_W['uniacid']));
		$yiwancheng = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' WHERE ordersn LIKE \'DH%\' AND uniacid = :uniacid AND status = 3', array(':uniacid' => $_W['uniacid']));
		$yiguanbi = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' WHERE ordersn LIKE \'DH%\' AND uniacid = :uniacid AND status = -1', array(':uniacid' => $_W['uniacid']));
		$all = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_order') . ' WHERE ordersn LIKE \'DH%\' AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid']));
		$json = json_encode(array($daifahuo, $weifukuan, $daishouhuo, $yiwancheng, $yiguanbi, $all));
		echo $json;
	}
	...
	protected function orderData($status, $st)
	{
		...
		if ($status !== '') {
			if ($status == '-1') {
				$statuscondition = ' AND o.status=-1 and o.refundtime=0 and ordersn LIKE \'DH%\' ';
			}
			else if ($status == '4') {
				$statuscondition = ' AND o.refundstate>0 and o.refundid<>0 and ordersn LIKE \'DH%\' ';
			}
			else if ($status == '5') {
				$statuscondition = ' AND o.refundtime<>0 and ordersn LIKE \'DH%\' ';
			}
			else if ($status == '1') {
				$statuscondition = ' AND ( o.status = 1 or (o.status=0 and o.paytype=3) ) and ordersn LIKE \'DH%\' ';
			}
			else if ($status == '0') {
				$statuscondition = ' AND o.status = 0 and o.paytype<>3 and ordersn LIKE \'DH%\' ';
			}
			else {
				$statuscondition = 'AND o.status = ' . intval($status) . ' and ordersn LIKE \'DH%\' ';
			}
		}
		else {
			$statuscondition = ' and ordersn LIKE \'DH%\' ';
		}
