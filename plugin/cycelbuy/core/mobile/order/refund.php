...
class Refund_EweiShopV2Page extends OrderMobilePage
{
	protected function globalData()
	{
		...
		$order = pdo_fetch('select id,status,price,refundid,goodsprice,dispatchprice,deductprice,deductcredit2,finishtime,isverify,`virtual`,refundstate,merchid,random_code from ' . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid and iscycelbuy=:iscycelbuy limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid, ':iscycelbuy' => 1));
