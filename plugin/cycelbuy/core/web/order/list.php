...
class List_EweiShopV2Page extends WebPage
{
	protected function orderData($status, $st)
	{
		...
		$condition = ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.istrade=0 and iscycelbuy = 1';
		$uniacid = $_W['uniacid'];
		...
		if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.istrade=0 ' || !empty($sqlcondition)) {
			...
			$sql = 'select * from ' . tablename('ewei_shop_order') . (' where uniacid = :uniacid and iscycelbuy = 1 and ismr=0 and deleted=0 and isparent=0 ' . $status_condition . ' GROUP BY id ORDER BY createtime DESC  ');
		...
		if (!empty($list)) {
			...
			foreach ($list as &$value) {
				...
				$order_goods = pdo_fetchall('select g.id,g.title,g.thumb,g.goodssn,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,
                    ...
                    og.diyformfields,op.specs,g.merchid,og.seckill,og.seckill_taskid,og.seckill_roomid,g.ispresell,g.costprice,op.costprice as option_costprice from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid ' . ' left join ' . tablename('ewei_shop_goods_option') . ' op on og.optionid = op.id ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $uniacid, ':orderid' => $value['id']));
				$goods = '';
...
';
		...
		if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)) {
			if ($searchfield == 'member') {
				...
				if (!empty($openidArr)) {
					...
					$orderPrice = pdo_fetch('SELECT count(1) as count,sum(price) as sumprice FROM ' . tablename('ewei_shop_order') . ' o WHERE o.uniacid = ' . $_W['uniacid'] . ' AND o.deleted=0 AND o.isparent=0 AND o.openid IN (' . $inOpenid . ')' . $priceStatus);
				}
				else {
			...
			$t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('ewei_shop_order') . (' WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 and iscycelbuy = 1 ' . $status_condition), $paras);
			$total = $t['count'];
			$totalmoney = $t['sumprice'];
		}
		...
		$r_type = array('退款', '退货退款', '换货');
		load()->func('tpl');
		include $this->template('cycelbuy/order/list');
	}
