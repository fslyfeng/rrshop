...
class Adv_EweiShopV2Page extends PluginWebPage
{
	...
	public function displayorder()
	{
		...
		$item = pdo_fetchall('SELECT id,advname FROM ' . tablename('ewei_shop_qa_adv') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);

		if (!empty($item)) {
			pdo_update('ewei_shop_qa_adv', array('displayorder' => $displayorder), array('id' => $id));
