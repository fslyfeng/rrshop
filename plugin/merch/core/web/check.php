...
class Check_EweiShopV2Page extends PluginWebPage
{
	protected function main($status = 1)
	{
		...
		$psize = 20;
		$condition = ' and u.uniacid=:uniacid and (creditstatus = 2 OR creditstatus = 0) ';
	...
	public function detail()
	{
		...
		$select = array();
		$select_order = pdo_fetchall('SELECT order_id FROM ' . tablename('ewei_shop_merch_bill_select') . ' WHERE bill_id =:bill_id', array(':bill_id' => $id));

		foreach ($select_order as $order) {
			$select[$order['order_id']] = 1;
		}
	...
	public function merchpay()
	{
		...
		if (empty($handpay) && $item['applytype'] == 2) {
			...
			if (!empty($sec['alipay_pay']['open'])) {
				...
				$single_res = m('finance')->singleAliPay(array('account' => $item['alipay'], 'name' => $item['applyrealname'], 'money' => $finalprice), $batch_no, $sec['alipay_pay'], '商户提现申请打款');
	...
	public function change_bill_select()
	{
		global $_W;
		global $_GPC;
		$id = intval($_GPC['id']);
		...
		if (!empty($id)) {
			if ($bpid == -1) {
				if (empty($type)) {
					pdo_delete('ewei_shop_merch_bill_select', array('bill_id' => $id));
				}
				else {
					$bill = pdo_fetch('SELECT orderids FROM ' . tablename('ewei_shop_merch_bill') . ' WHERE id = :id limit 1', array(':id' => $id));
					$orderids = iunserializer($bill['orderids']);
					$orderids = array_unique($orderids);

					foreach ($orderids as $ids) {
						pdo_insert('ewei_shop_merch_bill_select', array('bill_id' => $id, 'order_id' => intval($ids)), true);
					}
				}
			}
			else if (empty($type)) {
				pdo_delete('ewei_shop_merch_bill_select', array('order_id' => $bpid));
			}
			else {
				pdo_insert('ewei_shop_merch_bill_select', array('bill_id' => $id, 'order_id' => $bpid), true);
			}
		}
