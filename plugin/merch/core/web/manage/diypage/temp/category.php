...
class Category_EweiShopV2Page extends MerchWebPage
{
	public function main()
	{
		...
		$psize = 20;
		$list = pdo_fetchall('select id, name from ' . tablename('ewei_shop_diypage_template_category') . ' where merch=:merch and uniacid=:uniacid order by id desc limit ' . ($pindex - 1) * $psize . ',' . $psize, array(':merch' => intval($_W['merchid']), ':uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		if ($_W['ispost']) {
			...
			$item = pdo_fetch('SELECT id, name, uniacid FROM ' . tablename('ewei_shop_diypage_template_category') . ' WHERE merch=:merch and id=:id and uniacid=:uniacid ', array(':merch' => intval($_W['merchid']), ':uniacid' => $_W['uniacid'], ':id' => $id));
