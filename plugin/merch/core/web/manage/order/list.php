...
';
		...
		if ($searchfield == 'member') {
			...
			$orderPrice = pdo_fetch('SELECT count(1) as count,sum(o.price) as sumprice FROM ' . tablename('ewei_shop_order') . ' o left join ' . tablename('ewei_shop_member_address') . ' a on o.addressid = a.id WHERE o.uniacid = ' . $_W['uniacid'] . '  AND o.deleted=0 AND o.isparent=0 AND o.merchid = ' . $merchid . $priceCondition . $statuscondition);
			$totalmoney = $orderPrice['sumprice'];
			$total = $orderPrice['count'];
			...
			$t = pdo_fetch('SELECT count(DISTINCT(o.id)) as count,sum(o.price) as sumprice FROM ' . tablename('ewei_shop_order') . ' o ' . ' left join ' . tablename('ewei_shop_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('ewei_shop_merch_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid and s.merchid=o.merchid' . ' left join ' . tablename('ewei_shop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . (' ' . $sqlcondition . ' WHERE ' . $condition . ' ' . $statuscondition), $paras);
			$total = $t['count'];
