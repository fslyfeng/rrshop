...
class Log_EweiShopV2Page extends MerchWebPage
{
	...
	public function main()
	{
		...
		$psize = 20;
		$condition = ' d.uniacid = :uniacid and d.merchid = :merchid';
		$params = array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']);
		$couponid = intval($_GPC['couponid']);

		if (!empty($couponid)) {
			$coupon = pdo_fetch('select * from ' . tablename('ewei_shop_coupon') . ' where id=:id and uniacid=:uniacid and merchid=:merchid limit 1', array(':id' => $couponid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		if ($_GPC['export'] == 1) {
			...
			mplog('sale.coupon.log.export', '导出优惠券发放记录');
		}
