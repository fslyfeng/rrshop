...
class Adv_EweiShopV2Page extends PluginWebPage
{
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,advname FROM ' . tablename('ewei_shop_creditshop_adv') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
	...
	public function displayorder()
	{
		...
		$item = pdo_fetchall('SELECT id,advname FROM ' . tablename('ewei_shop_creditshop_adv') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
