	...
	public function exchange()
	{
		...
		if ($goods['isverify']) {
			if ($goods['verifytype'] == 0) {
				pdo_update('ewei_shop_creditshop_log', array('status' => 3, 'verifytime' => time(), 'verifyopenid' => $openid, 'time_finish' => time()), array('id' => $log['id']));
				$data = array('uniacid' => $uniacid, 'openid' => $log['openid'], 'logid' => $log['id'], 'verifycode' => $log['eno'], 'storeid' => $saler['storeid'], 'verifier' => $openid, 'isverify' => 1, 'verifytime' => time());
				pdo_insert('ewei_shop_creditshop_verify', $data);
			}
			else {
				if ($goods['verifytype'] == 1) {
					if ($log['status'] != 3) {
						pdo_update('ewei_shop_creditshop_log', array('status' => 3, 'usetime' => time(), 'verifyopenid' => $openid, 'time_finish' => time()), array('id' => $log['id']));
					}
