	...
	private function ujson_encode($array)
  {
	...
	public function main()
	{
		...
		if (is_error($auth)) {
			...
			$authUrl = EWEI_SHOPV2_AUTH_WXAPP . '/weixin/xcx/auth.html?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'].'&library='.LIBRARY_ID;
	...
	public function audit()
	{
		...
		if ($action == 'upload') {
			...
			$request = ihttp_post(EWEI_SHOPV2_AUTH_WXAPP . '/weixin/xcx/submit.html?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'] .'&library='.LIBRARY_ID, array('tabBar' => $tabBar));
		}
		else {
			$request = ihttp_post(EWEI_SHOPV2_AUTH_WXAPP . '/weixin/xcx/audit.html?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'] .'&library='.LIBRARY_ID, array());
		}
	...
	public function auth()
	{
		...
		header('Location:' . EWEI_SHOPV2_AUTH_WXAPP . '/weixin/xcx/wxauth.html?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'].'&library='. LIBRARY_ID );
	}
