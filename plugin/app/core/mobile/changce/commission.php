...
//小程序分销相关独家接口（小店设置、佣金排行等） 作者QQ:188872170
require EWEI_SHOPV2_PLUGIN . 'app/core/page_mobilev2.php';
class Commission_EweiShopV2Page extends AppMobilePage
{
	...
	public function rank()
	{
		...
		app_json(array('user'=>$user,'commission_title'=>$commission_title,'maxnum'=>intval($this->commission_rank['num'])));
	}
	public function rank_list()
	{
		...
		$psize = 20;
		$this->commission_rank = $_W['shopset']['commission']['rank'];
		if (empty($this->commission_rank) || empty($this->commission_rank['status'])) app_json(array('total' => 0, 'list' => array(), 'pagesize' => 0));
...
//print_r($this->commission_rank);exit;
		...
		app_json(array('total' => $total, 'list' => $result, 'pagesize' => $psize));
	...
	public function myshop()
	{
		...
		app_json(array('myshop' => $shop, 'share' => $_W['shopshare']));
	}
	public function myshopgoods()
	{
		...
		app_json(array('list' => $list['list'], 'total' => $list['total'], 'pagesize' => $args['pagesize']));
	}
}
