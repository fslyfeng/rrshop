	...
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			if ($dividend_ok <= 0 || empty($orders)) {
				return app_error(1, '参数错误,请刷新页面后重新提交!!');
			}

			$type = intval($_GPC['type']);

			if (!array_key_exists($type, $type_array)) {
				return app_error(1, '未选择提现方式，请您选择提现方式后重试!');
			}
			...
			if ($type == 2) {
				...
				if (empty($realname)) {
					return app_error(1, '请填写姓名!');
				}

				if (empty($alipay)) {
					return app_error(1, '请填写支付宝帐号!');
				}

				if (empty($alipay1)) {
					return app_error(1, '请填写确认帐号!');
				}

				if ($alipay != $alipay1) {
					return app_error(1, '支付宝帐号与确认帐号不一致!');
				}
				...
				if ($type == 3) {
					...
					if (empty($bankname)) {
						return app_error(1, '请选择银行!');
					}

					if (empty($bankcard)) {
						return app_error(1, '请填写银行卡号!');
					}

					if (empty($bankcard1)) {
						return app_error(1, '请填写确认卡号!');
					}

					if ($bankcard != $bankcard1) {
						return app_error(1, '银行卡号与确认卡号不一致!');
					}
