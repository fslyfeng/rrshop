...
class Adv_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			$data = array('uniacid' => $_W['uniacid'], 'advname' => trim($_GPC['advname']), 'link' => trim($_GPC['link']), 'enabled' => intval($_GPC['enabled']), 'displayorder' => intval($_GPC['displayorder']), 'thumb' => trim($_GPC['thumb']));

			if (!empty($id)) {
				pdo_update('ewei_shop_groups_adv', $data, array('id' => $id));
				plog('groups.adv.edit', '修改幻灯片 ID: ' . $id);
			}
			else {
				pdo_insert('ewei_shop_groups_adv', $data);
				$id = pdo_insertid();
				plog('groups.adv.add', '添加幻灯片 ID: ' . $id);
			}

			show_json(1, array('url' => webUrl('groups/adv')));
		}

		$item = pdo_fetch('select * from ' . tablename('ewei_shop_groups_adv') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		include $this->template();
	}
	...
	public function delete()
	{
		...
		foreach ($items as $item) {
			...
			plog('groups.adv.delete', '删除幻灯片 ID: ' . $item['id'] . ' 标题: ' . $item['advname'] . ' ');
		}
	...
	public function displayorder()
	{
		...
		if (!empty($item)) {
			...
			plog('groups.adv.delete', '修改幻灯片排序 ID: ' . $item['id'] . ' 标题: ' . $item['advname'] . ' 排序: ' . $displayorder . ' ');
		}
