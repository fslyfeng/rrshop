...
class Category_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$list = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_groups_category') . (' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY displayorder DESC'));
	...
	public function displayorder()
	{
		...
		$item = pdo_fetchall('SELECT id,name FROM ' . tablename('ewei_shop_groups_category') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
