		...
		if ($_W['ispost']) {
			...
			if (!empty($goods)) {
				...
				if (!empty($optionGoodsIDs)) {
					$optionGoodsData = pdo_fetchall('SELECT title,stock as total FROM ' . tablename('ewei_shop_goods_option') . ' WHERE goodsid IN  (:goodsid)', array(':goodsid' => implode(',', $optionGoodsIDs)));
				}
