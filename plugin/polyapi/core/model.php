...
class PolyapiModel extends PluginModel
{
	...
	public function GetOrder($array)
	{
		...
		if ($OrderStatus != 'JH_99') {
			...
			$condition .= ' and o.status=:status or(o.is_cashier=1 and o.status=3)';
		}
		...
		$sql = 'select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea,a.address as aaddress,
                  ...
                  r.rtype,r.status as rstatus,o.sendtype,o.is_cashier from ' . tablename('ewei_shop_order') . ' o' . ' left join ' . tablename('ewei_shop_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('ewei_shop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid ' . ' left join ' . tablename('ewei_shop_member_address') . ' a on a.id=o.addressid ' . ' left join ' . tablename('ewei_shop_dispatch') . ' d on d.id = o.dispatchid ' . ' left join ' . tablename('ewei_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('ewei_shop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . (' where ' . $condition . ' GROUP BY o.id ORDER BY o.createtime DESC  ');
		...
		if (!empty($list)) {
			foreach ($list as $k => $v) {
				...
				if (!empty($v['is_cashier']) && $v['status'] == 3) {
					$orders[$k]['tradeStatus'] = 'JH_02';
				}
	...
	public function Send($array)
	{
		...
		if (!empty($item['is_cashier']) && $item['status'] == 3) {
			return $return_data;
		}
	...
	public function SyncStock($array)
	{
		...
		if ($item['hasoption'] == 1) {
			...
			$change_num = $Quantity - intval($option['stock']);
			$goods_total = $item['total'] + $change_num;
