...
任务已完成，' . $score . $balance . $coupon . '剩余未发放奖励请到我的任务中领取
...
<a href=\'' . $url . '\'>点击查看详情</a>';
	...
	public function pickTask($taskid, $openid)
	{
		...
		if ($task['type'] == 'poster') {
			...
			if ($_W['ispost'] && !empty($_POST['openid'])) {
				$poster = $this->create_poster(array('id' => $taskArr['id'], 'design_data' => $taskArr['design_data'], 'design_bg' => $taskArr['design_bg'], 'stoptime' => $taskArr['stoptime'], 'poster_version' => $task['poster_version']));
	...
	public function send2wechat($recordid, $openid = '')
	{
		...
		if ($posterInfo['poster_version'] != $taskInfo['poster_version']) {
			$uploadResult = $this->createPoster2(array('id' => $posterInfo['recordid'], 'design_data' => $taskInfo['design_data'], 'design_bg' => $taskInfo['design_bg'], 'stoptime' => $taskInfo['stoptime'], 'poster_version' => $taskInfo['poster_version']), 0, true, true);
			$mediaid = $uploadResult['mediaid'];
			$posterInfo['mediaid'] = $mediaid;
			pdo_update('ewei_shop_task_qr', array('mediaid' => $mediaid, 'poster_version' => $taskInfo['poster_version']), array('id' => $posterInfo['id']));
		}
	...
	protected function getQR2($poster, $member)
	{
		...
		if (empty($qr)) {
			...
			pdo_insert('qrcode', $ims_qrcode);
			$qr = array('uniacid' => $_W['uniacid'], 'openid' => $member['openid'], 'sceneid' => $scene_id, 'ticket' => $result['ticket'], 'recordid' => $poster['id'], 'poster_version' => $poster['poster_version']);
...
';
		...
		m('notice')->sendNotice(array('openid' => $openid, 'tag' => $tag, 'default' => $message, 'cusdefault' => $text, 'url' => empty($notice_data['task_pick_template']) ? $url : '', 'datas' => $datas));
	}
...
';
		...
		$notice_data = m('common')->getSysset('notice', false);
		$datas = $this->getNoticeDatas($record);
		m('notice')->sendNotice(array('openid' => $openid, 'tag' => $tag, 'default' => $message, 'cusdefault' => $text, 'url' => empty($notice_data['task_progress_template']) ? $url : '', 'datas' => $datas));
	}
...
';
		...
		$notice_data = m('common')->getSysset('notice', false);
		$datas = $this->getNoticeDatas($record);
		m('notice')->sendNotice(array('openid' => $openid, 'tag' => $tag, 'default' => $message, 'cusdefault' => $text, 'url' => empty($notice_data['task_finish_template']) ? $url : '', 'datas' => $datas));
	}
...
<a href=\'' . $url . '\'>点击查看详情</a>';
		...
		m('notice')->sendNotice(array('openid' => $openid, 'tag' => $tag, 'default' => $message, 'cusdefault' => $text, 'url' => empty($notice_data['task_poster_pick_template']) ? $url : '', 'datas' => $datas));
	}
...
';
 ...
 <a href=\'' . $url . '\'>点击查看详情</a>';
		...
		$notice_data = m('common')->getSysset('notice', false);
		$datas = $this->getNoticeDatas($record, $this_member['nickname']);
		m('notice')->sendNotice(array('openid' => $openid, 'tag' => $tag, 'default' => $message, 'cusdefault' => $text, 'url' => empty($notice_data['task_poster_progress_template']) ? $url : '', 'datas' => $datas));
	}
...
感谢您的支持 <a href=\'' . $url . '\'>点击查看详情</a>';
		...
		$notice_data = m('common')->getSysset('notice', false);
		$datas = $this->getNoticeDatas($record, $this_member['nickname']);
		m('notice')->sendNotice(array('openid' => $openid, 'tag' => $tag, 'default' => $message, 'cusdefault' => $text, 'url' => empty($notice_data['task_poster_finish_template']) ? $url : '', 'datas' => $datas));
	}
...
';
 ...
 <a href=\'' . $url . '\'>点击查看详情</a>';
		...
		$notice_data = m('common')->getSysset('notice', false);
		$datas = $this->getNoticeDatas($record, $this_member['nickname']);
		m('notice')->sendNotice(array('openid' => $openid, 'tag' => $tag, 'default' => $message, 'cusdefault' => $text, 'url' => empty($notice_data['task_poster_scan_template']) ? $url : '', 'datas' => $datas));
	}
		...
		if (!empty($task['requiregoods'])) {
			...
			foreach ($goodsids as $key => $val) {
				...
				if (0 < $goods['hasoption']) {
					...
					if (0 < $goods['total'] || $goods['total'] == -1) {
						$enough = true;
					}
					else {
						$getmesg[$key] = $goods['title'] . '库存可能不充足.';
