...
class Credit_EweiShopV2Page extends WebPage
{
	protected function main($type = 'credit1')
	{
		...
		if ($_GPC['groupid'] || $_GPC['level'] || $_GPC['keyword']) {
			...
			$sql = 'select log.id,log.num,log.createtime,log.remark,log.credittype,u.username,\'0\' as uid,log.openid  from ' . tablename('ewei_shop_member_credit_record') . ' log ' . ' left join ' . tablename('users') . ' u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid' . (' where 1 ' . $condition . ' order by log.createtime desc');
		}
