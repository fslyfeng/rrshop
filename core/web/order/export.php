...
class Export_EweiShopV2Page extends WebPage
{
	...
	public function main()
	{
		...
		if ($_GPC['export'] == 1) {
			...
			$sql = 'select o.* , a.realname as arealname,m.level,a.mobile as amobile,a.province as aprovince ,a.city as acity,a.area as aarea, a.street as astreet,a.address as aaddress,d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,m.groupid,m.uid,sm.id as salerid,sm.nickname as salernickname,s.salername from ' . tablename('ewei_shop_order') . ' o' . ' left join ' . tablename('ewei_shop_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('ewei_shop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid ' . ' left join ' . tablename('ewei_shop_member_address') . ' a on a.id=o.addressid ' . ' left join ' . tablename('ewei_shop_dispatch') . ' d on d.id = o.dispatchid ' . ' left join ' . tablename('ewei_shop_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('ewei_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . (' where ' . $condition . ' ' . $statuscondition . ' ORDER BY o.createtime DESC,o.status DESC  ');
			$list = pdo_fetchall($sql, $paras);
			$userlevel = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_member_level') . ' WHERE uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
...
';
						...
						$value['goods_diyformdata'] = $og['goods_diyformdata'];
					}
				}
				...
				if (!empty($value['storeid'])) {
					if (0 < $value['merchid']) {
						$value['pickname'] = pdo_fetchcolumn('select storename from ' . tablename('ewei_shop_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $value['storeid']));
					}
					else {
						$value['pickname'] = pdo_fetchcolumn('select storename from ' . tablename('ewei_shop_store') . ' where id=:storeid limit 1 ', array(':storeid' => $value['storeid']));
					}
				}
