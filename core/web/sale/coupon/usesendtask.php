...
class Usesendtask_EweiShopV2Page extends ComWebPage
{
	...
	protected function post()
	{
		...
		if (!empty($id)) {
			$item = pdo_fetch('SELECT *  FROM ' . tablename('ewei_shop_coupon_usesendtasks') . ' WHERE uniacid = ' . $uniacid . ' and id =' . $id);

			if (!empty($item['usecouponid'])) {
				$usecoupon = pdo_fetch('SELECT id,couponname as title , thumb  FROM ' . tablename('ewei_shop_coupon') . ' WHERE uniacid = ' . $uniacid . ' and id =' . $item['usecouponid']);
			}
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				pdo_update('ewei_shop_coupon_usesendtasks', $data, array('id' => $id));
				plog('coupon.usesendtask.edit', '修改优惠券发送任务 ID: ' . $id);
			}
			else {
				pdo_insert('ewei_shop_coupon_usesendtasks', $data);
				$id = pdo_insertid();
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_coupon_usesendtasks') . ' WHERE id  = :id  and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));

		if (!empty($item)) {
			pdo_delete('ewei_shop_coupon_usesendtasks', array('id' => $id, 'uniacid' => $_W['uniacid']));
		}
