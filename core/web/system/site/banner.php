...
class Banner_EweiShopV2Page extends SystemPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			$data = array('title' => trim($_GPC['title']), 'background' => trim($_GPC['background']), 'thumb' => save_media($_GPC['thumb']), 'url' => trim($_GPC['url']), 'displayorder' => intval($_GPC['displayorder']), 'status' => trim($_GPC['status']));
