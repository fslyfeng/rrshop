...
class Category_EweiShopV2Page extends SystemPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_system_category') . ' ORDER BY displayorder asc');
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,name FROM ' . tablename('ewei_shop_system_category') . ' WHERE id = :id', array(':id' => $id));
