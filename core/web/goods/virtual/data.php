...
class Data_EweiShopV2Page extends ComWebPage
{
	...
	public function main()
	{
		...
		if ($_GPC['status'] == '2') {
			...
			$order = 'orderid ASC,is_top desc,sort_time desc,id ASC';
		}
			...
			if (!empty($typeid)) {
				...
				if (!empty($item['fields'])) {
					...
					if (!empty($noinsert)) {
						...
						show_json(1, array('message' => '部分数据保存成功！' . $tip, 'url' => webUrl('goods/virtual/data', array('typeid' => $typeid))));
					}
					else {
	...
	public function top()
	{
		...
		if (empty($id)) {
			show_json(0, '请选择要操作的数据');
		}

		$types = pdo_fetchall('SELECT id,is_top FROM ' . tablename('ewei_shop_virtual_data') . (' WHERE id in( ' . $id . ' ) AND typeid=' . $typeid . ' and merchid=0 and uniacid=') . $_W['uniacid']);

		if (empty($types)) {
			show_json(0, '操作的数据不存在 , 请刷新重试');
		}

		$time = time();

		foreach ($types as $key => $type) {
			$is_top = abs(1 - $type['is_top']);
			if ($op == 'up' && $is_top == 0 || $op == 'down' && $is_top == 1) {
				continue;
			}

			$update = array('is_top' => $is_top);

			if ($is_top) {
				$update['sort_time'] = $time + $key;
			}
			else {
				$update['sort_time'] = 0;
			}

			pdo_update('ewei_shop_virtual_data', $update, array('id' => $type['id']));
		}
