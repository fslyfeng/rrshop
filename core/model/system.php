		...
		$shopmenu = array(
			'shop'       => array(
				...
				'items'    => array(
					...
					array(
						'title' => '配送方式',
						'items' => array(
							array('title' => '普通快递', 'route' => 'dispatch', 'desc' => '店铺普通快递管理'),
							array('title' => '同城配送', 'route' => 'cityexpress', 'desc' => '店铺同城配送管理')
						)
					),
			...
			'goods'      => array(
				...
				'items'    => array(
					...
					array('title' => '固定信息', 'route' => 'fixedInfo'),
					array(
						'title' => '虚拟卡密',
						'route' => 'virtual',
						'iscom' => 'virtual',
				...
				'items'    => array(
					array(
						...
						'items' => array(
							...
							array('title' => '绑定送积分', 'route' => 'bindmobile')
						)
					),
					array(
						'title' => '优惠券',
				...
				'items'    => array(
					...
					array(
						...
						'items' => array(
							...
							array('title' => '消息通知队列', 'route' => 'notice_redis')
						)
					),
					array(
						'title' => '短信配置',
					...
					array(
						...
						'items' => array(
							...
							array('title' => '模板消息修复', 'route' => 'templatetool'),
							array('title' => '清空海报缓存', 'route' => 'postertool'),
							array('title' => '支付证书验证', 'route' => 'wxpaycert'),
							array('title' => '七牛修复', 'route' => 'fixQiniu')
						)
					),
					array(
						'title' => '入口',
		...
		if (!$this->merch) {
			...
			if (p('commission')) {
				...
				if ($this->cv('commission.apply.view2')) {
					$arr['commission2'] = pdo_fetchcolumn('select count(1) from' . tablename('ewei_shop_commission_apply') . ' a ' . ' left join ' . tablename('ewei_shop_member') . ' m on m.uid = a.mid' . ' left join ' . tablename('ewei_shop_commission_level') . ' l on l.id = m.agentlevel' . ' where a.uniacid=:uniacid and a.status=:status', array(':uniacid' => $_W['uniacid'], ':status' => 2));
				}
			}

			if ($this->cv('shop.comment')) {
				$arr['comment'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('ewei_shop_order_comment') . 'c LEFT JOIN ' . tablename('ewei_shop_goods') . ' g ON g.id=c.goodsid WHERE (c.checked=1 OR c.replychecked=1) AND c.deleted=0 AND c.uniacid=:uniacid AND g.merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => 0));
			}
		}
		else {
			$arr['notice'] = 'none';
