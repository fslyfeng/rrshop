...
class Pay_EweiShopV2Page extends MobileLoginPage
{
	...
	public function main()
	{
		...
		if ((empty($order['isnewstore']) || empty($order['storeid'])) && empty($order['istrade'])) {
			...
			if (!$seckillinfo) {
				$ret = m('order')->CheckoodsStock($orderid, 1, false);
				...
				if (!$ret) {
					if ($_W['ispost']) {
						...
						$this->message('您当前下单商品库存不足,请重新下单', mobileUrl('order'));
					}
				}
			}
		}
		...
		if (empty($log)) {
			$log = array('uniacid' => $uniacid, 'openid' => trim($member['uid']), 'module' => 'ewei_shopv2', 'tid' => trim($order['ordersn']), 'fee' => $order['price'], 'status' => 0);
		...
		$payinfo = array('orderid' => $orderid, 'ordersn' => trim($log['tid']), 'credit' => $credit, 'alipay' => $alipay, 'wechat' => $wechat, 'cash' => $cash, 'money' => $order['price']);

		if (is_h5app()) {
			$payinfo = array('wechat' => !empty($sec['app_wechat']['merchname']) && !empty($set['pay']['app_wechat']) && !empty($sec['app_wechat']['appid']) && !empty($sec['app_wechat']['appsecret']) && !empty($sec['app_wechat']['merchid']) && !empty($sec['app_wechat']['apikey']) && 0 < $order['price'] ? true : false, 'alipay' => false, 'mcname' => trim($sec['app_wechat']['merchname']), 'aliname' => empty($_W['shopset']['shop']['name']) ? trim($sec['app_wechat']['merchname']) : trim($_W['shopset']['shop']['name']), 'ordersn' => trim($log['tid']), 'money' => $order['price'], 'attach' => $_W['uniacid'] . ':0', 'type' => 0, 'orderid' => $orderid, 'credit' => $credit, 'cash' => $cash);
	...
	public function complete()
	{
		...
		if (!empty($ispeerpay)) {
			...
			if ($ispeerpay['peerpay_type'] == 0 && $ispeerpay['peerpay_realprice'] != $peerpay) {
				...
				if ($ispeerpay['peerpay_type'] == 1 && !empty($ispeerpay['peerpay_selfpay']) && $ispeerpay['peerpay_selfpay'] < $peerpay && 0 < floatval($ispeerpay['peerpay_selfpay'])) {
					if ($_W['ispost']) {
						show_json(0, '参数错误');
					}
					else {
		...
		if ($type == 'credit') {
			...
			$fee = floatval($ps['fee']);
			$result = m('member')->setCredit($openid, 'credit2', 0 - $fee, array($_W['member']['uid'], $_W['shopset']['shop']['name'] . '消费' . $fee . ',订单号:' . $order['ordersn']));
			...
			if (!empty($ispeerpay)) {
				...
				$peer_status = m('order')->peerStatus(array('pid' => $ispeerpay['id'], 'uid' => $member['id'], 'uname' => $member['nickname'], 'usay' => '', 'price' => $log['fee'], 'createtime' => time(), 'headimg' => $peerheadimg['avatar'], 'openid' => $peerheadimg['openid'], 'usay' => trim($_GPC['peerpaymessage'])));

				if (!$peer_status) {
					if ($_W['ispost']) {
						show_json(0, '代付信息写入失败,请重试! 如已支付请联系平台退款');
					}
					else {
						$this->message('代付信息写入失败,请重试! 如已支付请联系平台退款', mobileUrl());
					}
				}
			}
	...
	public function peerpay()
	{
		...
		$goods = pdo_fetchall('SELECT g.id,og.goodsid,og.total,g.title,g.thumb,g.type,g.status,og.price,og.title as gtitle,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,og.seckill,og.seckill_taskid,
                ...
                FROM ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on og.goodsid = g.id ' . ' left join ' . tablename('ewei_shop_goods_option') . ' op on og.optionid = op.id ' . (' where ' . $scondition . ' and og.uniacid=:uniacid order by og.id asc'), array(':id' => $orderid, ':uniacid' => $uniacid));
		$address = iunserializer($order['address']);
		$member = m('member')->getMember($openid, true);
	...
	public function peerpayshare()
	{
		...
		$_W['shopshare'] = array('title' => '我想对你说：' . $peerpay['peerpay_message'], 'imgUrl' => tomedia($goods['thumb']), 'desc' => $peerpay['peerpay_message'], 'link' => mobileUrl('order/pay/peerpaydetail', array('id' => $peerid, 'mid' => $member['id']), 1));
		include $this->template();
	}
	...
	public function peerpaydetail()
	{
		...
		$peerid = intval($_GPC['id']);
		$uniacid = $_W['uniacid'];
		$peerpay = pdo_fetch('select p.*,o.openid,o.address,o.isparent,o.id as oid from ' . tablename('ewei_shop_order_peerpay') . ' p join ' . tablename('ewei_shop_order') . ' o on o.id=p.orderid where p.orderid=:id and p.uniacid=:uniacid limit 1', array(':id' => $peerid, ':uniacid' => $uniacid));
		...
		$goods = pdo_fetchall('SELECT g.id,og.goodsid,og.total,g.title,g.thumb,g.type,g.status,og.price,og.title as gtitle,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,og.seckill,og.seckill_taskid,
                og.sendtype,og.expresscom,og.expresssn,og.express,og.sendtime,og.finishtime,og.remarksend
                FROM ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on og.goodsid = g.id ' . ' left join ' . tablename('ewei_shop_goods_option') . ' op on og.optionid = op.id ' . (' where ' . $scondition . ' and og.uniacid=:uniacid order by og.id asc'), array(':id' => $peerpay['orderid'], ':uniacid' => $uniacid));
