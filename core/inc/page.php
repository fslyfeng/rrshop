	...
	public function runTasks()
	{
		...
		if (p('groups')) {
			...
			if ($groups_team_lasttime + $groups_team_interval <= $groups_team_current) {
				...
				ihttp_request($_W['siteroot'] . ('addons/ewei_shopv2/plugin/groups/task/refund.php?uniacid=' . $_W['uniacid']), NULL, NULL, 1);
			}
		...
		if (p('friendcoupon')) {
			...
			if ($lasttime + $interval <= $current) {
				...
				ihttp_request($_W['siteroot'] . ('addons/ewei_shopv2/plugin/friendcoupon/task/sendMessage.php?uniacid=' . $_W['uniacid']), NULL, NULL, 1);
			}
		}
		...
		if (p('merch')) {
			...
			if ($lasttime + $interval <= $current) {
				...
				ihttp_request(EWEI_SHOPV2_TASK_URL . 'plugin/merch.php', NULL, NULL, 1);
			}
		}
	}
